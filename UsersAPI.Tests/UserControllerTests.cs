...
namespace UsersAPI.Tests
{
    public class UserControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public async void POST_AddUser_Test(string url)
        {
            ...
            var responseAdmin = await client.PostAsJsonAsync(url, userModels[3]); // добавляем второго админа получаем BadRequest(Ошибка создания.
        ...
        public async void DELETE_DeleteUser_Test()
        {
            ...
            // Act
            var responseUser = await client.PostAsJsonAsync("api/v1/users?login=admin&password=admin", userModel);// создаем пользователя чтобы потом удалить
        ...
        public async void GET_GetById_Test()
        {
            ...
            // Act
            var responseUser = await client.PostAsJsonAsync("api/v1/users?login=admin&password=admin",// создаем пользователя чтобы потом получить
